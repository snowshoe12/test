...
public class TokenAuthenticationFilter extends OncePerRequestFilter{
	 ...
	 * - HTTP 헤더를 통해서 전달 -> 웹앱, spa(리액트 등) 사용유저만 적용
	...
	protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain filterChain)
		...
		// 웹앱, SPA등 서비스에서 진입했을 때 => restapi 방식으로 진입했을 때
		...
		// 3. 액세스 토큰이 유효한가? 검사!!
		if(tokenProvider.isValidToken(accessToken)) { 	// 유효함 => 정상 처리 => 인증 정보 세팅하여 해당 페이지로 이동
			System.out.println("유효한 토큰");
			// 인증 정보 생성 후 -> 인증 정보 관리 객체(시큐리티 컨텍스트 홀더)에 추가, 1회성
			// 원하는 페이지로 이동
			filterChain.doFilter(request, response);
			...
			if (! tokenProvider.isValidToken(refreshToken)) {
				System.out.println("리프레시 토큰 자체에 문제가 존재함");
			...
			// 액세스 토큰 재발행 => 쿠키 저장 => 원하는 페이지로 이동
			// TokenService : refreshToken => user id
			// 순환 참조 오류(DI를 남발) 발생 시 다른 컨트롤러에서 해결하게 조정
